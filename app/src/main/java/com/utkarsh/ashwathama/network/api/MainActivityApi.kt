...
class MainActivityApi @Inject constructor(
    ...
    suspend fun getDeviceDetails(userId: String,passWord: String): IResult<LoginResponseModel?> {
        val getDeviceDetailsService = networkService.build(MainActivityInterface::class.java)
        return handleApiResponse(request = { getDeviceDetailsService.getDeviceDetails(userId,passWord) })
    }
